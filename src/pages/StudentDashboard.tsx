...
const StudentDashboard = () => {
  ...
  const fetchData = async () => {
    ...
    // Fetch my teams (as leader or member)
  ...
  const handleSearchFriend = async () => {
    ...
    const { data } = await supabase
      .from("profiles")
      .select("axon_id, name, college, email, phone, user_id")
      .ilike("axon_id", `%${friendSearch.trim()}%`)
      .limit(1)
      .single();
    setFriendResult(data as Profile | null);
            ...
            {activeTab === "hackathons" && (
              <motion.div key="hackathons" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                ...
                {selectedHackathon && !showRegForm && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-6 rounded-xl border border-border bg-card p-6">
                    ...
                    {selectedHackathon.brochure_url && (
                      <a href={selectedHackathon.brochure_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs font-mono text-muted-foreground hover:text-foreground mb-4">
                        ...
                        {teammateAxonIds.map((axon, i) => (
                          <div key={i} className="flex gap-2 mb-2">
                            <Input
                              value={axon}
                              onChange={e => {
                                const updated = [...teammateAxonIds];
                                updated[i] = e.target.value;
                                setTeammateAxonIds(updated);
                              }}
                              placeholder="AXN-XXXXXXXX"
                              className="bg-background border-border font-mono text-xs"
                            />
                            {teammateAxonIds.length > 1 && (
                              <Button type="button" variant="ghost" size="sm" onClick={() => setTeammateAxonIds(teammateAxonIds.filter((_, j) => j !== i))}>
                                <X className="w-3 h-3" />
                              </Button>
                            )}
                          </div>
              ...
              <motion.div key="friends" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                ...
                {friendResult === null && friendSearch && !friendSearching && (
                  <div className="rounded-xl border border-border bg-card p-8 text-center">
                    <p className="text-sm text-muted-foreground font-mono">No user found</p>
                  </div>
                )}
                  ...
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="rounded-xl border border-border bg-card p-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        {friendResult.email && <p className="text-xs text-muted-foreground">{friendResult.email}</p>}
                      </div>
                    </div>
                  </motion.div>
                )}
